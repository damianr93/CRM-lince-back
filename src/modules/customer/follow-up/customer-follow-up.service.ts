...
import { FollowUpTask } from './follow-up-task.schema';
import {
...
export class CustomerFollowUpService {
  ...
  private isAutomationEnabled(): boolean {
    return envs.FOLLOW_UP_AUTOMATION_ENABLED === true;
  }
  ...
  private buildMessage(templateId: FollowUpTemplateId, customer: Client): string {
  ...
  private resolveAssigneeEmail(
    ...
    const fallbackEmail =
      entry.email ||
      envs.FOLLOW_UP_NOTIFY_DEFAULT_EMAIL ||
      envs.FOLLOW_UP_NOTIFY_SIN_ASIGNAR_EMAIL ||
      envs.MAILER_EMAIL ||
      null;
  ...
  private normalizePhoneForWhatsApp(phone: string | null): string | null {
    ...
    // Si ya tiene formato completo con +549, devolverlo
    if (cleanPhone.startsWith('+549') && cleanPhone.length >= 13) {
      return cleanPhone;
    }

    // Si tiene formato 549, agregar +
    if (cleanPhone.startsWith('549') && cleanPhone.length >= 12) {
      return `+${cleanPhone}`;
    }

    // Si tiene formato 54, agregar +9
    ...
    // Si no se puede normalizar, devolver null
    return null;
  }
}
